...
void RectPS(float& X1, float& Y1, float& X2, float& Y2);//Функция Enter, с прееменной с плавающей запятой 

void Enter();//Функция RectPS
...
bool number(int a, int b, int c);//Функция number

void answer(bool pair);

// Функции задачи Integer22

void time();//Функция time

void result1(int N);//Функция result 
...
{
    {
        ...
        {
            case 1: Enter();//Конструкция case с функцией RectPS
...
void Enter()//Функция RectPS
{
    ...
    {
        RectPS(x11, y11, x21, y21);//Функция Enter
    }
    else//Оператор else
    {
    ...
    {
        RectPS(x12, y12, x22, y22);//Функция Enter
    }
    else//Оператор else
    {
        cout << " Неправильні координати "<< endl;// Показ в консоли
    ...
    {
        RectPS(x13, y13, x23, y23);//Функция Enter
    }
    else//Оператор else 
    {
        cout << " Неправильні координати "<< endl;// Показ в консоли
...
bool number(int a, int b, int c)//Функция number
{
    ...
    pair = ((a == b)||(b == c)||(c == a));//Переменная pair внутри которой решение задания
    return pair;
}
